...
function HODLR_trace_apply{T<:Number}(HK::KernelHODLR{T}, HKj::DerivativeHODLR{T}, vec::Vector{T})::T
  tmp1 = Array{T}(length(vec))
  tmp2 = Array{T}(length(vec))
  At_ldiv_B!(tmp1, HK.W, vec)
  A_mul_B!(  tmp2, HKj,  tmp1)
...
function HODLR_grad_term{T<:Number}(K::KernelMatrix{T}, HK::KernelHODLR{T}, dfn::Function, dat::Vector{T}, 
...
function stoch_gradient{T<:Number}(K::KernelMatrix{T}, HK::KernelHODLR{T}, dat::Vector{T},
                                   dfuns::Vector{Function}, vecs::Vector{Vector{T}};
                                   plel::Bool=false, verbose::Bool=false, shuffle::Bool=false)::Vector{T}
  # Test stuff:
...
function HODLR_p_grad_term{T<:Number}(K::KernelMatrix{T}, HK::KernelHODLR{T}, dfn::Function, dat::Vector{T},
                                      vecs::Vector{Vector{T}}; plel::Bool=false, verbose::Bool=false, seed::Int64=0)
  # Get the EXACT HODLR derivative:
  verbose && println("Assembling derivative matrix...")
  HKj       = DerivativeHODLR(K, dfn, HK, plel=plel)
  # Sequentially apply the solves/products:
...
# You need to supply a vector of derivative functions here, which is maybe a little wacky.
# But otherwise, pretty straightforward function.
function stoch_profile_gradient{T<:Number}(K::KernelMatrix{T}, HK::KernelHODLR{T}, dat::Vector{T},
                                           dfuns::Vector{Function}, vecs::Vector{Vector{T}};
                                           plel::Bool=false, verbose::Bool=false,
                                           shuffle::Bool=false)::Vector{T}
  # Test stuff:
  HK.U == nothing                  || error("The matrix needs to be factorized for this to work.")
  length(dfuns) == length(K.parms) || error("You didn't supply the right number of gradient funs.")
  # Shuffle the SAA vectors if requested:
