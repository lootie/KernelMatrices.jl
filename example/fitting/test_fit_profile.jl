...
# Set the equivalent options for the full likelihood:
...
println()
@time fitd = HODLR.trustregion(pinits, loc_s, dat_s, d2funs, popts, vrb=true, profile=true, rtol=1.0e-5)

# Now fit the full likelihood for comparison:
println()
println("Optimizing full likelihood...")
println()
@time fuld = HODLR.trustregion(finits, loc_s, dat_s, fd2funs, fopts, vrb=true, rtol=1.0e-5)
...
for j in eachindex(trup)
  println("\t $(round(fitd[j], 4))       $(round(fuld[j], 4))")
end
println()
