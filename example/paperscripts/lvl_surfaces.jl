...
srfs     = map(x->zeros(gdsz, gdsz), eachindex(lvls))
...
for j in eachindex(lvls)
  ...
  topts = HODLR.Maxlikopts(kernfun,dfuns,0.0,HODLR.FixedLevel(lvls[j]),rnk,HODLR.givesaa(35,5),false,false,false,true)
  try
...
# Now save everything:
using JLD
save("../../data/likelihood_surfaces.jld", "srfs", srfs, "data", dat_s, "locations", loc_s, "lvls", lvls,
